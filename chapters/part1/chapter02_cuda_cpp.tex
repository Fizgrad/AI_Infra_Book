...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \draw[arrow] (-3.8,-2.25) -- node[below] {控制流强} (-1.7,-2.25);
    \draw[arrow] (2.0,-2.25) -- node[below] {数据并行强} (5.8,-2.25);
  \end{tikzpicture}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[align=center] at (0,0.75) {同一条指令被 32 个线程同时执行，线程 id 不同，处理的数据位置不同};
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[align=left, text width=4.2cm] at (2.7,-1.15) {
      当某个 warp 等待内存时，\\
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[align=left, text width=4.3cm] at (5.0,0.35) {
      \textbf{性能问题定位：}\\
